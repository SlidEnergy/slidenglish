...
namespace SlidEnglish.Web
{
	public class Startup
	{
		...
		private void ConfigureInfrastructure(IServiceCollection services)
		{
			...
			if (CurrentEnvironment.IsDevelopment())
				googleCredential = GoogleCredential.FromFile(Path.Combine(CurrentEnvironment.ContentRootPath, "..", "google-translate-secret.json"));
			else
				googleCredential = GoogleCredential.FromJson(Environment.GetEnvironmentVariable("GOOGLE_APPLICATION_CREDENTIALS"));

			services.AddSingleton(x => googleCredential);

			services.AddSlidEnglishInfrastructure(connectionString);
		}
